...
export default function TenriChat() {
  ...
  useEffect(() => {
    // Auto-start chat directly without location verification
    loadScenes();
  }, []);
  ...
  // Load scenes data without starting chat immediately
  const loadScenes = () => {
    fetch("/tenriEn.json")
      ...
      .then((data) => {
        ...
        // Don't set chatStarted here - let user click start conversation
  ...
  // Function to start chat when user clicks start conversation
  const startChat = () => {
    setChatStarted(true);
    
    // Now load the initial scene
    if (scenes.length > 0) {
      const initialScene = scenes.find((scene) => scene.id === 0);
      if (initialScene) {
  ...
  const showNextProgressiveMessage = () => {
    ...
    console.log("showNextProgressiveMessage called:", {
      currentScene,
      currentProgressiveIndex,
      totalMessages: currentSceneMessages.length,
      currentMessage: currentSceneMessages[currentProgressiveIndex]
    });
    
    // Safety check: if no messages for current scene, stop execution
    if (!currentSceneMessages || currentSceneMessages.length === 0) {
      console.log("No progressive messages for current scene", currentScene, "- stopping execution");
    ...
    if (currentProgressiveIndex >= currentSceneMessages.length) {
      console.log("Index out of bounds, resetting progressive system");
      setIsShowingProgressive(false);
      setCurrentProgressiveIndex(0);
      return;
    }
    ...
    // Check if message has text or image property
    if (!currentMessage.text && !currentMessage.image) {
      console.warn("Message has no text or image property:", currentMessage);
      ...
      if (currentProgressiveIndex < currentSceneMessages.length - 1) {
        ...
        console.log("Skipping invalid message, moving to next at index", nextIndex);
        setTimeout(() => {
          console.log("Skip timeout triggered, moving to index", nextIndex);
    ...
    const newMessage = {
      ...
      // Add image property if it exists
    ...
    console.log("Added message:", {
      text: chosenText ? chosenText.substring(0, 50) + "..." : "Image message",
      expectInput: currentMessage.expectInput,
      delay: currentMessage.delay
    });

    // Check if this message expects input
    if (currentMessage.expectInput) {
      console.log("Message expects input, pausing progression");
    ...
    if (currentProgressiveIndex < currentSceneMessages.length - 1) {
      const nextIndex = currentProgressiveIndex + 1;
      console.log("Scheduling next message in", currentMessage.delay || 2000, "ms", "to index", nextIndex);
      setTimeout(() => {
        console.log("Timeout triggered, moving to next message at index", nextIndex);
        setCurrentProgressiveIndex(nextIndex);
      }, currentMessage.delay || 2000);
    } else {
      console.log("All messages shown for scene", currentScene, "- stopping progressive mode");
      // All messages shown, stop progressive mode
      setIsShowingProgressive(false);
      setCurrentProgressiveIndex(0);
      
             // Only show buttons when naturally completing the scene (not after interruption)
       // This ensures the suggestion box appears at the natural end of the scene
       if (!wasInterrupted) {
         console.log("Scene completed naturally - showing suggestion box");
         setMessages(prev => {
         ...
         console.log("Scene completed after interruption - NOT showing suggestion box");
  ...
  useEffect(() => {
    ...
    console.log("Progressive effect triggered:", {
      isShowingProgressive,
      currentProgressiveIndex,
      currentScene,
      currentSceneMessagesLength: currentSceneMessages.length
    });
    ...
    if (isShowingProgressive && currentSceneMessages.length > 0 && currentProgressiveIndex < currentSceneMessages.length) {
      ...
      console.log("Calling showNextProgressiveMessage from effect");
  ...
  const getRandomScript = (scene) => {
    ...
    return scene.script || "Script not available";
  };
  ...
  const updateCurrentSceneData = (sceneId) => {
    ...
    console.log("updateCurrentSceneData called with sceneId:", sceneId, "found sceneData:", sceneData);
    if (sceneData) {
      ...
      if (sceneData.yesAction) {
        ...
        console.log("Setting yes button text:", sceneData.yesAction.text);
      }
      if (sceneData.noAction) {
        buttonTexts.no = sceneData.noAction.text;
        console.log("Setting no button text:", sceneData.noAction.text);
      }
      if (sceneData.otherAction) {
        buttonTexts.other = sceneData.otherAction.text;
        console.log("Setting other button text:", sceneData.otherAction.text);
      }
      console.log("Final buttonTexts:", buttonTexts);
      setCurrentButtonTexts(buttonTexts);
    } else {
      console.log("No scene data found for sceneId:", sceneId);
    }
  };

  // Helper function to create media messages
  const createMediaMessages = (mediaArray) => {
    console.log("createMediaMessages called with:", mediaArray);
    const messages = mediaArray.map((media) => {
      ...
      console.log("Created media message:", message);
      return message;
    });
    console.log("All media messages created:", messages);
    return messages;
  };
  ...
  const handleYesNoResponse = async (response, messageIndex) => {
     ...
     if (response === "Yes" && sceneData?.yesAction?.text) {
       buttonText = sceneData.yesAction.text;
     } else if (response === "No" && sceneData?.noAction?.text) {
    ...
    if (scenes.length === 0) {
      console.log("Scenes not loaded yet, waiting...");
      setIsLoading(false);
      return;
    }
    ...
    let action;
      ...
      if (!action) {
        console.log("No noAction found, skipping to next scene");
    ...
    if (action && action.type === "redirect") {
      ...
      if (currentScene === 11) {
        console.log('Scene 11 detected, showing survey modal before redirect');
    ...
    if (action && action.type === "nextScene") {
      ...
      console.log("Looking for scene ID:", action.target);
      ...
      console.log("Target scene media:", targetSceneData?.media);
      console.log(
        "Target scene script preview:",
        targetSceneData?.script ? targetSceneData.script.substring(0, 100) : "No script available"
      );
    }
    ...
    if (
      ...
      console.log(
        "Creating media messages for target scene:",
        targetSceneData.media
      );
      newMessages = createMediaMessages(targetSceneData.media);
      console.log("Media messages created:", newMessages);
    } else {
      console.log(
        "No media messages created. Response:",
        response,
        "Target scene data:",
        targetSceneData
      );
         ...
         // Add conditional response for "No" - only if noAction exists
     // Removed the "Alright then" message
    ...
    let scriptMessage = null;
        ...
        if (targetSceneData.messages && targetSceneData.messages.length > 0) {
          ...
          console.log("Scene", action.target, "has", targetSceneData.messages.length, "progressive messages - starting in 150ms");
    ...
    if (scriptMessage) {
      console.log("Adding messages with script:", [...newMessages, scriptMessage]);
      setMessages((prev) => [...prev, ...newMessages, scriptMessage]);
    } else {
      // Only add media messages if no script message
      console.log("Adding only media messages:", newMessages);
      setMessages((prev) => [...prev, ...newMessages]);
    }

             // Handle scene transition
    if (action && action.type === "nextScene") {
      console.log("Transitioning to scene", action.target);
      setCurrentScene(action.target);
      // Update current scene data and button texts for new scene
      updateCurrentSceneData(action.target);
             setConversationState(
         `scene${action.target}_after_${response === "Yes" ? "yes" : "no"}`
       );
    } else {
  ...
  const resumeProgressiveMessages = () => {
    ...
    console.log("resumeProgressiveMessages called:", {
      currentScene,
      currentProgressiveIndex,
      totalMessages: currentSceneMessages.length
    });
    ...
    // Safety check: if no messages for current scene, stop execution
    if (!currentSceneMessages || currentSceneMessages.length === 0) {
      console.log("No progressive messages for current scene", currentScene, "- stopping resume");
      setIsShowingProgressive(false);
      setCurrentProgressiveIndex(0);
      return;
    }
    ...
    if (currentProgressiveIndex < currentSceneMessages.length - 1) {
      console.log("Resuming progressive messages for scene", currentScene, "- moving to next message");
      // Move to next message index first
      setCurrentProgressiveIndex(prev => prev + 1);
      // Then enable progressive mode
      setIsShowingProgressive(true);
    } else {
      console.log("No more messages to show for scene", currentScene);
      // Only show buttons if this is a natural completion, not after interruption
      // Don't automatically show buttons when resuming - let the natural flow handle it
    }
  };
  ...
  const sendMessage = async () => {
    ...
    if (
      ...
      if (sceneMatch) {
        ...
        console.log(
          "Frontend: Scene determined from conversation state:",
          actualCurrentScene
        );
      }
    }
    ...
    try {
      console.log("Frontend - Sending to AI route:", {
        ...
        setMessages((prev) => [...prev, aiMessage]);
          ...
          setTimeout(() => {
            const sceneQuestionMessage = {
              role: "assistant",
              content: "Alright, now let's continue with the story. What would you like to do?",
              timestamp: new Date().toLocaleTimeString(),
      ...
      const errorMessage = {
        role: "assistant",
        content: `${t.errorMessage} ${error.message}. ${
        ...
        <div className="flex flex-col h-screen max-w-4xl mx-auto md:max-w-6xl lg:max-w-7xl xl:max-w-8xl">
           ...
           <div className="bg-white border-b border-gray-200 px-4 md:px-6 lg:px-8 py-4 md:py-6 justify-between border-b">
            <div className="flex gap-4 items-center ">
              <img
                src="/images/tenri-avatar.png"
                alt="logo"
                className="w-10 h-10"
              />
               <img
                src="/images/tenri-tribal.png"
                alt="logo"
              ...
              <div key={index}>
                ...
                >
                  <div className="flex gap-2 md:gap-3 items-end">
                    ...
                    />
                    <div
                      className={`max-w-[90%] md:max-w-[80%] lg:max-w-[70%] rounded-lg px-3 py-3 md:px-4 md:py-4 tracking-wider ${
                        message.role === "user"
                    ...
                    >
                       ...
                       {message.video && (
                         <div className="mt-1">
                           <video
                            ...
                            controls
                            preload="metadata"
                            onLoadStart={() => console.log("Video loading started:", message.video)}
                            onLoadedData={() => console.log("Video loaded successfully:", message.video)}
                            onError={(e) => {
                              console.error("Video error:", e);
                              console.error("Video source:", message.video);
                            }}
                          >
                     ...
                     index === latestAssistantIndex && (
                      <div className="flex flex-col gap-[1svh] w-full mt-2">
                       ...
                       {console.log("Rendering suggestion box with:", { currentSceneData, currentButtonTexts })}
                       <div className="flex flex-wrap gap-[1svh] md:gap-[1vw] lg:gap-[0.8vw] justify-center md:justify-start">
                          ...
                          {currentSceneData?.yesAction && (
                                                       <button
                             onClick={() => handleYesNoResponse("Yes", index)}
                             className="bg-[#DBE8F5] text-black px-[2svh] md:px-[2vw] lg:px-[1.5vw] py-[0.5svh] md:py-[0.4vw] lg:py-[0.3vw] rounded-full font-full font-medium focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-colors hover:bg-[#C5D8E8] text-sm md:text-base lg:text-lg hover:scale-105 transform duration-200"
                           >
                             {currentButtonTexts.yes || "Yes"}
                           </button>
                          )}
                          {/* No option - only show if noAction exists */}
                          {currentSceneData?.noAction && (
                                                       <button
                             onClick={() => handleYesNoResponse("No", index)}
                          ...
                          {currentSceneData?.otherAction && (
                           ...
                           >
                             {currentButtonTexts.other || "Chat with AI"}
                           </button>
                          )}
                       </div>
                     </div>
          ...
          <div className="bg-[#A3A2A2] px-3 py-4 md:px-6 md:py-6 lg:px-8 lg:py-8">
            <div className="flex space-x-2 md:space-x-4 lg:space-x-6 items-center">
              ...
              <button
                ...
                className={`px-3 py-2 md:px-4 md:py-2 lg:px-6 lg:py-3 rounded-lg font-medium transition-colors text-sm md:text-base ${
                  messages.length > 0 && 
                  messages[messages.length - 1]?.expectInput 
              ...
              >
                  ...
                  messages[messages.length - 1]?.expectInput 
                    ...
                    : t.send}
                         ...
                         {/* Show input prompt when expecting input */}
             {messages.length > 0 && 
              messages[messages.length - 1]?.expectInput && (
          ...
          <div className="h-screen relative text-center flex flex-col items-center justify-center">
            ...
            <div className="mb-[7svh] md:mb-[5vh] lg:mb-[4vh]">
              <img src="/images/tenri-tribal.png" className="w-[25svw] md:w-[20vw] lg:w-[15vw]" />
            </div>

            <div className="mb-[4svh] md:mb-[3vh] lg:mb-[2vh]">
              <img src="/images/tenri.png" className="h-[25svh] md:h-[20vh] lg:h-[15vh]" />
            </div>
